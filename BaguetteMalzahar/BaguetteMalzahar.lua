...
function Malzahar:OrbStatus(bool)
	if _G.AutoCarry then
		_G.AutoCarry.MyHero:AttacksEnabled(bool);
		_G.AutoCarry.MyHero:MovementEnabled(bool);
	elseif _G.MMA_IsLoaded then
		if bool == true then 
			_G.MMA_AvoidMovement(); -- if Attack = true then MMA reset so he can attck
			_G.MMA_StopAttacks();
		else
			_G.MMA_AvoidMovement(false); -- else no
...
function Malzahar:PredLoader()
	...
	if _G.Reborn_Loaded ~= nil or _G.MMA_IsLoaded or _Pewalk then else
		self:LoadNEOrb();
	end
end
